...
import { Search, Clock, Layers, SlidersHorizontal, Plus, Bookmark } from 'lucide-react';
    ...
    <div className="absolute inset-0 flex flex-col w-full h-full">
      <header className="bg-white border-b border-gray-200/50 px-4 pt-3 pb-3 flex flex-col gap-2 shrink-0">
        <div className="text-left flex justify-between items-center w-full">
          ...
          <span className="text-[7px] font-mono uppercase bg-amber-50 border border-amber-200 text-amber-700 font-black px-2 py-0.5 rounded-md tracking-wider">
            {activeTab === 'search' && 'Buscar Recetas'}
            {activeTab === 'mine' && 'Mis Creaciones'}
            {activeTab === 'saved' && 'Mis Marcadores'}
          </span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4 flex flex-col items-stretch w-full pb-36 bg-stone-50/50">
        {loading ? (
          ...
          recetas.map((receta) => {
            ...
            return (
              <div key={receta.id} onClick={() => onSelectReceta(receta)} className="w-full bg-white border border-gray-200 rounded-2xl shadow-xs overflow-hidden flex flex-col text-left shrink-0 relative transition-transform active:scale-[0.99]">
                <div className="w-full h-40 bg-stone-200 relative">
                  ...
                  <div className="absolute top-2 left-2 flex flex-wrap gap-1 max-w-[70%]">
                    {receta.categorias_ids?.map(catId => mapaCategorias[catId] && (
                      <span key={catId} className="bg-black/60 text-white font-sans text-[7px] font-black uppercase px-1.5 py-0.5 rounded flex items-center gap-0.5"><Layers className="w-2 h-2 text-amber-400" /> {mapaCategorias[catId]}</span>
                    ))}
                  </div>
                  
                  {/* Bloqueo estricto solicitado: No puedo guardar mis propias recetas */}
                  {!esMia && (
                    <button type="button" onClick={(e) => onToggleSave(e, String(receta.id))} className="absolute top-2 right-2 p-1.5 rounded-full bg-white/90 shadow-md border focus:outline-none">
                ...
                <div className="p-3.5 bg-white flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-[10px] text-stone-500 mt-1 line-clamp-2 italic">{receta.descripcion || 'Sin descripción disponible.'}</p>
                  </div>
          ...
          <div className="py-16 text-center text-stone-400 text-xs font-bold w-full bg-white rounded-2xl border border-dashed p-6">
            ...
            {activeTab === 'mine' && 'Aún no has creado ninguna receta propia.'}
            {activeTab === 'saved' && 'No tienes ninguna receta guardada en tus marcadores.'}
          </div>
        )}
      </div>

      {/* 🔥 Subimos los botones de acción flotantes para que no tapen la barra inferior mágica */}
